...
// Modified slightly for namespacing, etc
...
#ifdef _MSC_VER
#define SMF_NOINLINE(...) __declspec(noinline) __VA_ARGS__
#else
#define SMF_NOINLINE(...) __VA_ARGS__ __attribute__((noinline))
#endif
...
struct KeyOrValueHasher : functor_storage<size_t, hasher> {
  ...
  explicit KeyOrValueHasher(const hasher &hash) : hasher_storage(hash) {}
  size_t
...
struct KeyOrValueEquality : functor_storage<bool, key_equal> {
  ...
  explicit KeyOrValueEquality(const key_equal &equality)
...
struct sherwood_v3_entry {
  static constexpr sherwood_v3_entry
  special_end_entry() {
...
struct sherwood_v3_entry_constexpr {
  static constexpr sherwood_v3_entry_constexpr
  special_end_entry() {
...
class sherwood_v3_table : private EntryAlloc, private Hasher, private Equal {
 ...
 private:
  ...
  template <typename Key, typename... Args>
  SMF_NOINLINE(std::pair<iterator, bool>)
...
struct prime_number_hash_policy {
  ...
  void
  commit(uint8_t new_prime_index) {
...
struct power_of_two_std_hash : std::hash<T> {
  typedef smf::power_of_two_hash_policy hash_policy;
};
