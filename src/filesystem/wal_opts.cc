...
wal_opts::wal_opts(wal_opts &&o) noexcept
  : directory(std::move(o.directory)), log_type(std::move(o.log_type)) {}

wal_opts::wal_opts(const wal_opts &o)
  : directory(o.directory), log_type(o.log_type) {}
...
std::ostream &operator<<(std::ostream &o, const wal_opts &opts) {
  o << "wal_opts{directory=" << opts.directory << ",log_type=" << opts.log_type
    << "}";
  return o;
}
}  // namespace smf
