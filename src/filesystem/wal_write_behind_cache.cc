...
wal_read_reply wal_write_behind_cache::get(const wal_read_request &req) {
  ...
  while (offset < max_offset() && bytes_left > 0) {
    ...
    f->hdr = std::make_unique<wal::wal_header>(*(it->second->hdr.get()));
    f->fragment.resize(it->second->fragment.size());
    std::memcpy(&f->fragment[0], it->second->fragment.get(),
                it->second->fragment.size());
    r.data->gets.push_back(std::move(f));
  }
  DLOG_DEBUG_IF(r.empty(), "Request from cache was not found.");
  if (!r.empty()) { r.data->next_offset = offset; }
  return r;
}
