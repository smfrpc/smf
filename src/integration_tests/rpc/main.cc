...
class rpc_client_wrapper {
 public:
  ...
  future<> connect() {
    smf::DLOG_DEBUG("Creating connections");
  ...
  future<uint64_t> send_request() {
    ...
    return connect()
      ...
      .then([begin, this] {
        ...
        smf::DLOG_DEBUG("Test took: {}ms", duration_milli);
        smf::DLOG_DEBUG("Queries per millisecond: {}/ms", qps);
...
class storage_service : public smf_gen::fbs::rpc::SmfStorage {
  future<smf::rpc_envelope> Get(
    ...
    // smf::DLOG_DEBUG("Server got payload {}", rec.get()->name()->size());
...
int main(int args, char **argv, char **env) {
  ...
  smf::DLOG_DEBUG("About to start the RPC test");
  ...
  return app.run(args, argv, [&app, &stats, &rpc, &clients,
    ...
    smf::DLOG_DEBUG("Setting up at_exit hooks");
    ...
    return stats.start()
      ...
      .then([&rpc, &stats, port] {
        ...
        return rpc.start(&stats, port, flags).then([&rpc] {
          smf::DLOG_DEBUG("Registering smf_gen::fbs::rpc::storage_service");
      ...
      .then([&rpc] {
        smf::DLOG_DEBUG("Invoking rpc start on all cores");
        return rpc.invoke_on_all(&smf::rpc_server::start);
      })
      .then([&clients, &config] {
        smf::DLOG_DEBUG("About to start the client");
      ...
      .then([&clients] {
        return clients
          ...
          .then([](auto ms) {
            smf::DLOG_DEBUG("Total time for all requests: {}ms", ms);
            return make_ready_future<>();
          });
      })
      .then([&clients] {
        smf::DLOG_DEBUG("MapReducing stats");
        return clients
          .map_reduce(adder<smf::histogram>(),
                      &rpc_client_wrapper::all_histograms)
          .then([](smf::histogram h) {
            smf::DLOG_DEBUG("Writing client histograms");
      ...
      .then([&] {
        return rpc
          ...
          .then([](smf::histogram h) {
            smf::DLOG_DEBUG("Writing server histograms");
            return smf::histogram_seastar_utils::write_histogram(
              "server_hdr.txt", std::move(h));
          });
      })
      .then([] {
        smf::DLOG_DEBUG("Exiting");
