      ...
      return seastar::do_with(seastar::semaphore(4), [f](auto &limit) {
        return seastar::do_for_each(
                 ...
                 [f, &limit](auto &c) mutable {
                   ...
                   // ON PURPOSE - testing to see if you can skip the first page
                   ...
                   return limit.wait(1).then([c, f, &limit] {
                     ...
                     f->dma_write(
                        ...
                        smf::priority_manager::get().default_priority())
...
write_poem_wal_segment(seastar::sstring base_dir, uint32_t max_appends = 100) {
  auto ws = seastar::make_lw_shared<smf::wal_segment>(
    base_dir + "/" + segment_name,
    smf::priority_manager::get().streaming_write_priority());
  return ws->open()
    .then([ws, max_appends] {
      return seastar::do_for_each(
        boost::counting_iterator<uint32_t>(0),
...
read_fully(const seastar::sstring &name) {
  ...
  try {
    ...
    return app.run(args, argv, [&] {
      ...
      return skip_first_page(dir)
        ...
        .then([dir] {
          return read_fully(dir + "/" + ostream_name).then([dir](auto obuf) {
            return read_fully(dir + "/" + segment_name).then([
              ostream_buf = std::move(obuf), &dir
            ](auto segment) {
              ...
              LOG_THROW_IF(
                ...
                "our wal_segment writes and seastar's ostream<char> do not "
