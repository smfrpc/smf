 ...
 *
 * layout
 * [ 8bits(compression) + 8bits(bitflags) + 16bits(session) + 32bits(size) + 32bits(checksum) +
 * 32bits(meta) ] total = 128bits == 16bytes
 *
 */
public final class Header extends Struct {
  ...
  public static int
  createHeader(FlatBufferBuilder builder,
    byte compression,
    byte bitflags,
    int session,
    long size,
