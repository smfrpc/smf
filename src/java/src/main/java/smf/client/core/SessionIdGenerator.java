 ...
 * restrictions. SessionIdGenerator is Thread Safe and can be used from multiple threads - the only
 * one restriction is that, it should be issued once per SMF-Client to prevent session collision
 * inside single JVM. Performance : this is separate issue that will be fixed in
...
public class SessionIdGenerator {
  ...
  /**
   * Generate new sessionId not used by any pending request. Method is thread safe and doesn't
   * block. Internally sessionId is assigned in "compare and set" manner, and if this assignment
   * will fail {@param MAX_RETRIES} times, SessionIdGenerationException exception will be thrown.
   *
   * @return sessionId that can be used by RPC request.
   * @throws SessionIdGenerationException if method cannot generate sessionId because of thread
   * contention.
   */
  public int
  next() {
    ...
    while (true) {
      /**
       * Even if sessionIdGen will turn over and two threads will get the same number, putIfAbsent
       * is atomic, and one of those threads will fail.
       */
