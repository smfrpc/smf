...
namespace smf {
static seastar::logger _smf_logger("smf");
...
#define SET_LOG_LEVEL(level) smf::_smf_logger.set_level(level)
#define __FILENAME__ \
  (strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__)
#define LOG_INFO(format, args...) \
  smf::_smf_logger.info("{}:{}] " format, __FILENAME__, __LINE__, ##args)
#define LOG_ERROR(format, args...) \
  smf::_smf_logger.error("{}:{}] " format, __FILENAME__, __LINE__, ##args)
#define LOG_WARN(format, args...) \
  smf::_smf_logger.warn("{}:{}] " format, __FILENAME__, __LINE__, ##args)
#define LOG_DEBUG(format, args...) \
  smf::_smf_logger.debug("{}:{}] " format, __FILENAME__, __LINE__, ##args)
#define LOG_TRACE(format, args...) \
  smf::_smf_logger.trace("{}:{}] " format, __FILENAME__, __LINE__, ##args)
...
#define LOG_INFO_IF(condition, format, args...)                              \
  do {                                                                       \
    if (condition) {                                                         \
      smf::_smf_logger.info("{}:{}] (" #condition ") " format, __FILENAME__, \
                            __LINE__, ##args);                               \
    }                                                                        \
  } while (0)
#define LOG_ERROR_IF(condition, format, args...)                              \
  do {                                                                        \
    if (condition) {                                                          \
      smf::_smf_logger.error("{}:{}] (" #condition ") " format, __FILENAME__, \
                             __LINE__, ##args);                               \
    }                                                                         \
  } while (0)
#define LOG_DEBUG_IF(condition, format, args...)                              \
  do {                                                                        \
    if (condition) {                                                          \
      smf::_smf_logger.debug("{}:{}] (" #condition ") " format, __FILENAME__, \
                             __LINE__, ##args);                               \
    }                                                                         \
  } while (0)
#define LOG_WARN_IF(condition, format, args...)                              \
  do {                                                                       \
    if (condition) {                                                         \
      smf::_smf_logger.warn("{}:{}] (" #condition ") " format, __FILENAME__, \
                            __LINE__, ##args);                               \
    }                                                                        \
  } while (0)
#define LOG_TRACE_IF(condition, format, args...)                              \
  do {                                                                        \
    if (condition) {                                                          \
      smf::_smf_logger.trace("{}:{}] (" #condition ") " format, __FILENAME__, \
                             __LINE__, ##args);                               \
    }                                                                         \
  } while (0)
