...
///
template <typename T> struct rpc_filter { seastar::future<T> operator()(T t); };
...
seastar::future<Ret...> rpc_filter_apply(const Iterator &b,
                                         const Iterator &end,
                                         Arg &&          arg) {
  auto begin = b;
  if (begin == end) {
...
seastar::future<Arg> rpc_filter_apply(Container *c, Arg &&arg) {
